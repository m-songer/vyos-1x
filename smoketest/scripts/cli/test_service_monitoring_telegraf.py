...
class TestMonitoringTelegraf(VyOSUnitTestSHIM.TestCase):
    ...
    def test_08_agent_configuration(self):
        ...
        self.cli_set(base_path + ['agent', 'collection-jitter', "25h"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_delete(base_path + ['agent', 'collection-jitter'])

        self.cli_set(base_path + ['agent', 'flush-interval', "25h"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_delete(base_path + ['agent', 'flush-interval'])

        self.cli_set(base_path + ['agent', 'flush-jitter', "25h"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_delete(base_path + ['agent', 'flush-jitter'])

        self.cli_set(base_path + ['agent', 'interval', "25h"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_delete(base_path + ['agent', 'interval'])
